...
public class SipdroidEngine implements RegisterAgentListener {
	...
	// add by ares
	public static final String ACTION_REGISTRATION_EVENT = "register or unregister action";
	...
	public boolean StartEngine() {
		...
		for (UserAgentProfile user_profile : user_profiles) {
			...
			try {
				...
				String version = "UUTALK/" + Sipdroid.getVersion() + "/"
						+ Build.MODEL;
	...
	public void unregister() {
		for (int i = 0; i < user_profiles.length; i++) {
			Log.d("$$$", "user_profiles " + i + " - username = "
					+ user_profiles[i].username + " , password = "
					+ user_profiles[i].passwd + " , from url = "
					+ user_profiles[i].from_url + " and sip provider = "
					+ sip_providers[i].getViaAddress());
	...
	public void onUaRegistrationFailure(RegisterAgent reg_ra,
		...
		if (retry
				...
				&& ((pwl[i] != null && !pwl[i].isHeld()) || (wwl[i] != null && !wwl[i]
						.isHeld())) && Receiver.on_wlan) {
