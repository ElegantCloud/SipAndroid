...
public class Receiver extends BroadcastReceiver {
		...
		if (call_state != state) {
			...
			case UserAgent.UA_STATE_IDLE:
				...
				if (wl != null && wl.isHeld())
					wl.release();
				// mContext.startActivity(createIntent(/*InCallScreen*/OutgoingCallActivity.class));
				// // modified by ares
				// add by ares
			...
			case UserAgent.UA_STATE_HOLD:
				...
				if (InCallScreen.started) // mContext.startActivity(createIntent(/*InCallScreen*/OutgoingCallActivity.class));
											// // modified by ares
				{
					// add by ares
					Log.d("Receiver", "UA_STATE_HOLD");
					Intent hold_intent = new Intent(
							SipdroidEngine.ACTION_INVITE_EVENT);
					hold_intent.putExtra(SipdroidEngine.EXTRA_EMBEDDED, "hold");
					mContext.sendBroadcast(hold_intent);
				}
	...
	public static void onText(int type, String text, int mInCallResId, long base) {
		Log.d("Receiver", "onText - type = " + type + " , text = " + text
				+ " , call resource id = " + mInCallResId + " and base = "
				+ base);
			...
			if (type == MISSED_CALL_NOTIFICATION) {
				...
				default:
					if (type >= REGISTER_NOTIFICATION
						...
						notification.contentIntent = PendingIntent.getActivity(
								mContext, 0,
								createIntent(/* Sipdroid */DemoActivity.class),
								0); // modified by ares
	...
	public static void moveTop() {
		progress();
		// mContext.startActivity(createIntent(/*Activity2*/OutgoingCallActivity.class));
		// // modified by ares
		// add by ares
		Log.d("Receiver", "moveTop");
	}
			...
			if (PreferenceManager.getDefaultSharedPreferences(mContext)
				...
				if (configurations != null) {
					...
					if (bestconfig != null
							...
							&& (activeSSID == null || activescan != null)
							&& asu(bestscan) > 22) {
						if (!Sipdroid.release)
